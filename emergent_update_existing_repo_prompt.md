...
**Your Goal:**
Please **analyze my existing codebase**, particularly how I am querying my database and structuring my frontend. Do NOT initialize a new project from scratch. Instead, carefully refactor and append the following new features into my existing architecture:

**1. Refactor Authentication & Role-Based Routing:**
...
- *Ensure you reuse my existing UI components where possible to maintain the current design language.*

**3. Build "MOD Mode" (Manager On Duty) into the Manager Dashboard:**
- Managers should see tasks assigned by the GM. Add a prominent **"MOD Mode"** button on their dashboard.
...
**4. Staff Task Verification (The "After" Photo):**
...
**5. Database Updates:**
- Please review my `schema.sql` (if provided) or existing database structure.
- Write and execute (or provide) the necessary SQL migration commands to support:
  - Role, department, and assigned location fields on the `Users` table.
  - A `Locations` table (if it doesn't exist).
  - `before_photo_url` and `after_photo_url` columns on the `Tasks` (or equivalent requests) table.

**Rules for Modifying the Codebase:**
- Do not blindly overwrite working logic—integrate these new features carefully.
- Maintain my current styling (Tailwind CSS/custom CSS). Ensure new components feel like they belong in the current app.
